...
bound range ($\beta_\mathrm{scale}\in[-0.2,0.2]$) for 6 shots (3
near-degenerate multimodal, 3 typical multimodal) and compared
$\mathrm{NLL}_\mathrm{profiled}(\beta)=\min_\phi \mathrm{NLL}(\phi,\beta)$
against a fine-grid $\mathrm{NLL}_\mathrm{marginalised}(\beta)=
-\log\int e^{-\mathrm{NLL}(\phi,\beta)}\,d\phi$ (stable log-sum-exp over
the same 360-point grid used to characterise the multimodality). Result:
the $\arg\min_\beta$ shift between the profiled-sum and marginalised-sum
objectives was \textbf{exactly 0.0000} at the sweep's grid resolution,
for both the full 6-shot sum and the near-degenerate-only subset. Only
1 of 6 shots exhibited even a single $\phi$-basin switch across the whole
sweep, and that switch did not move the summed optimum. Per-shot, profiled
and marginalised NLL differ by only $\sim\!4$ units (out of $\sim\!1.5
\times10^7$) when the dominant basin doesn't change -- consistent with
marginalisation contributing only a near-$\beta$-independent Laplace
normalisation offset in that regime, which cannot move an argmin.
...
$\beta$ magnitudes this problem actually explores, because basin-switching
within that range is rare. (Caveat: this is a 6-shot, 1-direction check,
not exhaustive; a full-dataset version would strengthen confidence
further, but the result is consistent and has a clean explanation.) No
marginalisation fix (e.g.\ a multi-start Laplace-mixture) is warranted.
...
fix) is statistically adequate for this estimator despite the underlying
$\phi$ landscape being universally multimodal. The remaining open item is
Issue 3's outer-loop budget sensitivity and the \code{grad\_tol}
calibration -- neither changes $\hat\beta$'s validity qualitatively, but
both should be tightened before treating any single \code{outer\_maxiter=15}
run's $\hat\beta$ as fully converged.
