...
This section derives, rigorously and explicitly, why estimating $\beta$
reduces to (1) a per-shot, per-plane estimate of $\theta_{iz}$ from the
image, followed by (2) a signal-fit step that only needs those estimates
(and the raw pixel counts) as input -- not the images themselves. The
notation matches \code{notes/joint\_profile\_inference.tex}
...
regression). \textbf{Crucially, Eq.~\ref{eq:beta-mle} does not need the raw
images at all once $\hat\theta_{iz}$ and the total per-pixel counts are
in hand} -- the entire image-processing burden is front-loaded into
estimating $\theta_{iz}$ per shot, and every method compared in this note
differs \emph{only} in how that front-loaded estimate is produced.
...
\begin{table}[htbp]
\centering
\caption{Kinematic parameter RMSE by method and atom count. Position
...
\midrule
$\mu_{x0}$      & $1.017\times10^{-5}$ & $1.017\times10^{-5}$ & $9.28\times10^{-6}$ & $9.28\times10^{-6}$ \\
$\mu_{y0}$      & $9.68\times10^{-6}$  & $9.68\times10^{-6}$  & $9.30\times10^{-6}$ & $9.29\times10^{-6}$ \\
$\mu_{vx0}$     & $2.70\times10^{-6}$  & $2.64\times10^{-6}$  & $2.48\times10^{-6}$ & $2.40\times10^{-6}$ \\
$\mu_{vy0}$     & $2.56\times10^{-6}$  & $2.50\times10^{-6}$  & $2.47\times10^{-6}$ & $2.41\times10^{-6}$ \\
$\sigma_x$      & $1.019\times10^{-5}$ & $1.019\times10^{-5}$ & $9.87\times10^{-6}$ & $9.88\times10^{-6}$ \\
$\sigma_y$      & $1.000\times10^{-5}$ & $9.99\times10^{-6}$  & $9.83\times10^{-6}$ & $9.82\times10^{-6}$ \\
$\sigma_{vx}$   & $8.92\times10^{-6}$  & $\mathbf{7.05\times10^{-7}}$ & $8.91\times10^{-6}$ & $\mathbf{6.70\times10^{-7}}$ \\
$\sigma_{vy}$   & $8.83\times10^{-6}$  & $\mathbf{6.91\times10^{-7}}$ & $8.97\times10^{-6}$ & $\mathbf{6.73\times10^{-7}}$ \\
\bottomrule
\end{tabular}
\end{table}
...
budget. This is consistent with the mechanism proposed in
\S\ref{sec:map-moments}: the moment method only observes the detection-plane
position variance $V_{xf}=V_{x0}+2T C_{xv0}+T^2 V_{vx0}$, a single scalar
mixing three unknowns, so recovering $V_{vx0}$ specifically is weak and
prior-dominated regardless of photon count; the pixel likelihood instead
resolves the conditional-velocity structure directly from the fine-grained
pixel pattern (the ``Pixel ACS'' section of \code{joint\_profile\_inference.tex}),
which is a photon-count-independent geometric effect.
...
\begin{table}[htbp]
\centering
\caption{Signal $(A_s,A_c)$ RMSE by method and atom count, $N=10$ independent runs.}
\label{tab:beta-rmse}
\begin{tabular}{lrrrr}
\toprule
& \multicolumn{2}{c}{$10^6$ atoms} & \multicolumn{2}{c}{$10^8$ atoms} \\
Method & RMSE($A_s$) & RMSE($A_c$) & RMSE($A_s$) & RMSE($A_c$) \\
\midrule
Null              & 0.001412 & 0.000737 & 0.000641 & 0.001078 \\
MAP-moments       & 0.001284 & 0.000636 & 0.000566 & 0.000936 \\
Pixel-likelihood (best) & $\mathbf{0.000495}$ & $\mathbf{0.000321}$ & $\mathbf{0.000074}$ & $\mathbf{0.000086}$ \\
Oracle            & 0.000468 & 0.000304 & 0.000032 & 0.000020 \\
\midrule
...
\begin{tabular}{lrr}
\toprule
Configuration & RMSE($\sigma_{vx}$) & RMSE($\sigma_{vy}$) \\
\midrule
bins=16, native range (baseline)  & $8.69\times10^{-7}$ & $8.60\times10^{-7}$ \\
bins=32, native range             & $7.14\times10^{-7}$ & $6.63\times10^{-7}$ \\
bins=16, tight range              & $7.25\times10^{-7}$ & $6.81\times10^{-7}$ \\
bins=32, tight range (both)       & $6.99\times10^{-7}$ & $6.34\times10^{-7}$ \\
\bottomrule
\end{tabular}
\end{center}
