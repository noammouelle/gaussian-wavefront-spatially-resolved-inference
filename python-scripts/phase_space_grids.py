...
-------
  ...
  --wtype            'confocal' (default, analytic) or 'interpolated'
                      (sampled wavefront from optics/generate_wavefront.py)
  --beam_file_down    HDF5 field for the downward beam (wtype=interpolated only)
  --beam_file_up      HDF5 field for the upward beam (wtype=interpolated only)
  --tag               Output filename tag (default: 'CONFOCAL_FINE' for
                      wtype=confocal; required for wtype=interpolated so
                      different wavefronts don't collide on disk)

For wtype=interpolated, run optics/convergence_check.py against the same
--beam_file_up/--beam_file_down grid resolution first -- there is no default
resolution that's correct for every wavefront.
"""
...
def main():
    ...
    p.add_argument('--wtype', choices=['confocal', 'interpolated'], default='confocal')
    p.add_argument('--beam_file_down', type=str, default=None,
                   help='HDF5 field for the downward beam (wtype=interpolated only)')
    p.add_argument('--beam_file_up', type=str, default=None,
                   help='HDF5 field for the upward beam (wtype=interpolated only)')
    p.add_argument('--tag', type=str, default=None,
                   help="output filename tag (default: 'CONFOCAL_FINE' for "
                        "wtype=confocal; required for wtype=interpolated)")
    args = p.parse_args()
    ...
    if args.wtype == 'interpolated':
        if not (args.beam_file_down and args.beam_file_up):
            p.error('--wtype interpolated requires both --beam_file_down and --beam_file_up')
        if not args.tag:
            p.error('--wtype interpolated requires --tag, so different wavefronts '
                     'get distinct output filenames')
        beam_file = (os.path.abspath(args.beam_file_up), os.path.abspath(args.beam_file_down))
        tag = args.tag
    else:
        beam_file = None
        tag = args.tag or 'CONFOCAL_FINE'
    ...
    print(f'  wtype            : {args.wtype}')
    if args.wtype == 'interpolated':
        print(f'  beam_file_up     : {beam_file[0]}')
        print(f'  beam_file_down   : {beam_file[1]}')
    print()

    stems = {z0: f'PSGRID4D_{tag}_Z{int(z0)}' for z0 in Z0_VALUES}
    ...
    for z0 in Z0_VALUES:
        ...
        param_dict = build_param_dict(xgrid, ygrid, zgrid, vxgrid, vygrid, vzgrid,
                                       wtype=args.wtype, beam_file=beam_file)
    ...
    log_run(os.path.join(script_dir, '..'), 'phase_space_grids', tag,
            wtype=args.wtype, beam_file_up=beam_file[0] if beam_file else None,
            beam_file_down=beam_file[1] if beam_file else None,
            nx=args.nx, ny=args.ny, nvx=args.nvx, nvy=args.nvy,
            xrange=args.xrange, vxrange=args.vxrange,
            aisi_files=[os.path.join(indir, s + '.aisi') for s in stems.values()])
