...
N_SHOTS shots, for a given dataset (1e6 or 1e8 atoms). Also saves raw counts
needed for the downstream beta-fit step, so nothing needs recomputing.

Usage: python generate_kinematic_estimates.py <1e6|1e8> <n_runs> <n_shots>
"""
...
out_path = REPO / 'results' / f'kinematic_estimates_{DATASET_KEY}_N{N_RUNS}_shots{N_SHOTS}.json'
...
for run_dir in run_dirs:
    ...
    print(f'{DATASET_KEY} {run_name}: t={t_run:.0f}s  (total elapsed {time.perf_counter()-t_start:.0f}s)', flush=True)
